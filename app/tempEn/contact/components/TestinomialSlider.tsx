...
export default function TestimonialSlider() {
  ...
  const t = useTranslations(messages?.contact?.testimonials || {});
  ...
  useEffect(() => {
    const startAnimation = async () => {
      const slideWidth = firstRow.length * 406; // 400px card width + 6px gap
      await Promise.all([
  ...
  }, [controlsFirstRow, controlsSecondRow, messages]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <Link href={contactPath}>
            <Button className="bg-transparent text-white border-2 border-white hover:bg-white hover:text-[#051238] transition-colors rounded-md px-8 py-2">
              {t("button")}
            </Button>
          </Link>
        </div>
