	...
	const plans = [
		{
			...
			originalPrice: t("pricing.comparisonPlans.plans.starter.originalPrice"),
			...
			bottomText: t("pricing.comparisonPlans.plans.starter.bottomText"),
		},
		{
			...
			originalPrice: t("pricing.comparisonPlans.plans.essential.originalPrice"),
			...
			bottomText: t("pricing.comparisonPlans.plans.essential.bottomText"),
		},
		{
			...
			originalPrice: t("pricing.comparisonPlans.plans.premium.originalPrice"),
			...
			bottomText: t("pricing.comparisonPlans.plans.premium.bottomText"),
		},
		{
			...
			bottomText: t("pricing.comparisonPlans.plans.enterprise.bottomText"),
		},
	];
	...
	return (
		<>
			<section className='py-24 bg-[#F8FAFF]'>
				<div className='2xl:max-w-screen-2xl mx-auto px-4'>
          ...
          {t("pricing.comparisonPlans.title")}
					...
					<div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8'>
						{plans.map((plan, index) => (
							<div
								key={index}
								className={`bg-white rounded-3xl p-8 min-w-[328px] flex flex-col justify-between gap-10 ${
									index === 2 ? "ring-2 ring-[#284fe6]" : ""
								}`}
							>
										...
										{plan?.features?.map(
											(feature: string, featureIndex: number) => (
												...
												>
													...
													<span className={`text-sm text-[#081F4D] leading-tight pr-9 ${index !== 0 && featureIndex === 0 ? 'font-bold' : ''}`}>
														{feature}
													</span>
												</div>
											)
								...
								<span className='text-center text-xs text-[#6B7280] leading-tight mt-auto'>
									{plan.bottomText}
								</span>
							</div>
						))}
